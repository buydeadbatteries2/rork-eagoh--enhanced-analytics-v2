...
// Open Intelligence service imports removed — Sessions now navigates to the shared
// /open-intelligence route instead of rendering a duplicate inline implementation.
import {
...
export default function SessionsScreen(): JSX.Element {
  ...
  const handleSessionPress = useCallback((session: SessionType): void => {
    ...
    // Open Intelligence: navigate to the shared full-featured route
    // instead of rendering a duplicate inline implementation.
    if (session.id === "open-intelligence") {
      router.push({
        pathname: "/open-intelligence",
        params: selectedEagohId ? { eagohId: selectedEagohId } : {},
      } as never);
      return;
    }
  ...
  // Setup view — Open Intelligence navigates to its own route
  if (activeSession) {
...
const styles = StyleSheet.create({
  ...
  // Open Intelligence (shared styles used by Faction Network & My Rankings sessions)
